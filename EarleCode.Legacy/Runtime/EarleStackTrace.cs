            ...
            foreach (var frame in Frames)
            {
                ...
                switch (callerIP)
                {
                    ...
                    default:
                    {
                        ...
                        var callerFunctionArgs = string.Join(",",
                            frame.ParentFrame?.Function?.Parameters ?? new string[0]);

                        var line = 0;
                        frame.ParentFrame?.Function?.CallLines?.TryGetValue(callerIP, out line);

                        result +=
                            $" at {callerFunctionName}({callerFunctionArgs}) [0x{callerIP:X5}] in {callerFileName}:{line}";
